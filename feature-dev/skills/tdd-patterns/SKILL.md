...
- **Maximum 5 RED-GREEN cycles** per bounded behavior (one acceptance criterion), not per feature. A feature with N criteria gets N separate budgets. If tests still fail after 5 cycles on the same criterion, halt and report the blocking issue; the orchestrator decides whether to re-dispatch.
