...
### Changed
- Trimmed `claude-md-best-practices` skill description to fit Claude Code's skill-listing budget.
...
### Added
- Added `hooks` to all commands (`analyze-claude-md`, `optimize-claude-md`, `init`) for next step guidance

### Improved
- Updated `claude-md-best-practices` skill description with "when NOT to use" guidance
- Skill now explicitly states it should NOT be used for general prompt engineering or non-CLAUDE.md files
...
### Fixed
- Use explicit path instead of wildcards for skills in plugin.json
  - Changed from `./skills/*/SKILL.md` to `./skills/claude-md-best-practices/SKILL.md`
  - Wildcard patterns not supported by plugin system
  - Resolves persistent "skills path not found" error
...
### Fixed
- Fixed skills path in plugin.json from `./skills/*/claude-md-best-practices` to `./skills/*`
  - Corrected wildcard pattern to match actual directory structure
  - Plugin now loads skills correctly without path resolution errors
